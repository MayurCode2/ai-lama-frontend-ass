...
import { deleteEpisode } from "./projectSlice";
import { useNavigate } from "react-router-dom";
import { setSelectedEpisode } from "./projectSlice";
...
function Episodes() {
    ...
    const [tableData, setTableData] = useState([]);
      ...
      // Update the tableData state when the selectedProject changes
  useEffect(() => {
    // Check if selectedProject is available and it has episodes
    if (selectedProject && selectedProject.episodes) {
      // Map the episodes to the format expected by the Table component
      const episodesData = selectedProject.episodes.map((episode) => ({
        id: episode._id, // Assuming each episode has a unique identifier
        name: episode.name,
        uploadDateTime: episode.dateTime, // Update with the actual property name
        status: episode.status,
        description: episode.description
      }));
      console.log(episodesData.status)

      // Set the episodes data to the tableData state
      setTableData(episodesData);
    }
  }, [selectedProject]);
  ...
  const handleEdit = async (id) => {
    const selectedEpisode = tableData.find((item) => item.id === id);
    console.log("Selected Episode:", selectedEpisode); // Add this log
    await dispatch(setSelectedEpisode(selectedEpisode)); // Add this log
    navigate("/transcript");
  };

  const handleDelete = async (id) => {
    try {
      // Assuming you have access to the selected project ID
      const projectId = selectedProject._id;

      // Dispatch the deleteEpisode action
      await dispatch(deleteEpisode({ projectId, episodeId: id }));
  ...
  return (
    <div className="flex">
      ...
      <div className="md:w-3/4  lg:w-3/4 ml-10">
        ...
        <div>
          <h1 className=" text-primaryblue text-2xl font-bold">{selectedProject.title}</h1>
        </div>
       {/* grid card */}
        <div className="flex flex-wrap">
          <div className="flex border border-secondarybg rounded-xl p-1 px-6 mt-5  cursor-pointer">
          ...
          <div className="flex border border-secondarybg rounded-xl p-1 px-6 ml-5 mt-5 mr-5 cursor-pointer">
            <div className="bg-primaryblue rounded-full  mr-3">
              <svg
               className=" w-12 h-12"
                xmlns="http://www.w3.org/2000/svg"
            ...
            <div className="flex justify-center flex-col ">
              ...
              <p className="text-xs">Spotify Podcast </p>
            </div>
          </div>
          <div className="flex border border-secondarybg rounded-xl p-1 px-6 mt-5  cursor-pointer">
            <div className="bg-primaryblue rounded-full  mr-3">
              <img   className=" w-12 h-12" src={spotify} alt="" />
            </div>
            <div className="flex justify-center flex-col ">
          ...
          <div className=" bg-primaryblue rounded-xl text-white flex justify-between mt-5 h-16  mr-14"><p className=" mt-5 ml-6">All files are processed! Your widget is ready to go!</p><button className=" bg-white text-btnblack px-4 my-3 rounded-lg mr-8">Try it out</button></div>
